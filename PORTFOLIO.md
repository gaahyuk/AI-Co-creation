...
| 8 | **테스트 1초 로그인** | 원클릭 데모 로그인 시 테스터 계정 즉석 자동 가입 및 4축 디폴트 프로필 자동 주입 | P0 | ✅ 구현 완료 |
| 9 | **어드민 모드** | 사이트 내에서 실시간으로 정책 데이터를 추가/삭제하며 매칭 알고리즘을 즉석 검증하는 대시보드 | P0 | ✅ 구현 완료 |
...
- [x] 테스트용 원클릭 데모 로그인 및 즉석 자동 가입/기본 매칭 데이터 주입 기능 구현
- [x] 어드민 모드 대시보드(/admin) 구현 (실시간 정책 데이터 추가, 조회, 삭제 CRUD 연동)
- [x] profiles 스키마 is_admin 컬럼 추가 및 레이아웃 사이드바 권한 제어 연동
...
├── src/
│   ├── app/
│   │   ├── admin/            # 어드민 모드 (실시간 정책 CRUD)
│   │   ├── auth/             # 인증 (1초 데모 로그인 및 일반 회원가입/로그인)
