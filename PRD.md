...
# 7. 정책 데이터 파이프라인 및 AI 정제 아키텍처 (Data Pipeline & AI Refinement)

정확하고 신속한 정책 추천 서비스를 위해 본 플랫폼은 **"자동 수집 ➔ AI 구조화 정제 ➔ 어드민 검증 ➔ 로컬 DB 캐싱"**의 4단계 정책 파이프라인을 핵심 데이터 아키텍처로 삼습니다.

```mermaid
flowchart TD
    A[외부 데이터 소스<br>공공API / 크롤링] -->|1. 정기 수집| B(배치 스케줄러<br>Supabase Cron)
    B -->|2. 비구조화 텍스트 전달| C[AI Parser<br>Gemini / LLM]
    C -->|3. JSON 구조화 및 정제| D[로컬 DB 임시 적재<br>policies 테이블 - draft]
    D -->|4. 데이터 검증 및 신뢰도 확보| E[어드민 대시보드<br>/admin]
    E -->|5. 최종 승인 & 노출| F[실서비스 반영<br>policies 테이블 - published]
    F -->|6. 초고속 매칭 진단| G[일반 사용자 대시보드]
```

### 1단계: 멀티 채널 데이터 수집 (Data Ingestion)
*   **공공 API 수집**: 공공데이터포털(data.go.kr)의 '중앙부처/지자체 복지서비스 API' 및 중소벤처기업부의 '기업마당(Bizinfo) API'를 연동하여 정기적으로 공고를 호출합니다.
*   **지자체 사이트 크롤링**: API로 적재되지 않는 지자체 고시공고 및 지원금 소식을 웹 크롤러를 통해 백그라운드 수집합니다.

### 2단계: AI 기반 정책 구조화 정제 (AI Refinement)
*   **비구조화 텍스트 해석**: 수집된 원본 텍스트 및 공고문 문서(PDF/HWP)는 자격 요건이 복잡한 줄글 형태로 되어 있습니다.
*   **LLM 파싱 (AI Parser)**: LLM(Gemini API 등)을 활용해 줄글 형태의 원본에서 **나이 범위(min_age, max_age), 지원 지역(locations), 대상 직업(jobs), 소득 상한선(income_limit), 필수 구비 서류(required_documents)** 등의 핵심 자격 조건을 정밀 가공하여 DB 스키마에 일치하는 **구조화된 JSON 데이터**로 추출합니다.

### 3단계: 어드민 검증 워크플로우 (Admin Verification Workflow)
*   **Draft 임시 적재**: AI가 자동 정제한 데이터는 즉시 서비스에 반영되지 않고, 신뢰도 확보를 위해 `status = 'draft'` 상태로 DB에 비활성 적재됩니다.
*   **어드민 페이지 검토**: 관리자 대시보드(`/admin`)의 미승인 목록 뷰에서 관리자는 AI가 가공한 조건들을 빠르게 검토 및 수정할 수 있습니다.
*   **최종 승인 및 배포**: 관리자가 최종 승인 버튼을 누르면 `status = 'published'`로 변경되어 일반 사용자용 매칭 진단 엔진에 실시간으로 포함됩니다.

### 4단계: 스케줄링 및 DB 캐싱 (Scheduling & Caching)
*   **새벽 배치 동기화**: 외부 API 및 크롤러 기동은 하루 1회(새벽 시간대) Supabase Cron 및 Edge Functions를 활용해 백그라운드에서 백배치(Back-batch)로 안전하게 돌립니다.
*   **초고속 로컬 매칭**: 외부 API를 매 요청마다 실시간 조회하는 병목을 없애고 로컬 DB에 데이터를 동기화/캐싱해 둠으로써, 사용자 대시보드 로딩 시 **0.1초 미만의 초고속 진단 연산**을 보장합니다.

---

## 7.2 AI Requirements (AI 연동 사양 - 로드맵 단계)

## AI Module 1: AI Chat Advisor (정책 상담사)
...
erDiagram
    ...
    policies {
        ...
        text status "상태 (draft: 임시/승인대기, published: 승인/노출중)"
        timestamp created_at
    }
...
*   **위험**: 정부 정책은 수시로 신설 및 변경되므로 구식 정보 매칭 시 신뢰가 하락할 수 있으며, AI 자동 정제 과정에서 데이터 오류(Hallucination 등)가 생기면 사용자가 자격을 오인할 위험이 있음.
*   **대비책**: **"정기 배치 크롤러 ➔ AI 구조화 Parser ➔ 어드민 최종 검증(Draft to Publish)"** 파이프라인을 가동합니다. 모든 크롤링 데이터는 임시 승인대기(`draft`) 상태로 축적하고, 관리자가 어드민 대시보드(`/admin`)에서 한 차례 검증 및 검수 조정한 후 승인 처리하도록 설계하여 100%에 수렴하는 정책 정합성과 신뢰도를 확보합니다.
